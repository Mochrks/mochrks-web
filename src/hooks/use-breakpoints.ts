...
export default function useBreakpoints() {
  ...
  const [isSm, setIsSm] = useState<boolean>(window.matchMedia(breakpoints.sm).matches);
  const [isMd, setIsMd] = useState<boolean>(window.matchMedia(breakpoints.md).matches);
  const [isLg, setIsLg] = useState<boolean>(window.matchMedia(breakpoints.lg).matches);
  const [isXl, setIsXl] = useState<boolean>(window.matchMedia(breakpoints.xl).matches);
  const [is2xl, setIs2xl] = useState<boolean>(window.matchMedia(breakpoints["2xl"]).matches);
  ...
  useEffect(() => {
    ...
    const handleSmChange = (event: MediaQueryListEvent) => setIsSm(event.matches);
    const handleMdChange = (event: MediaQueryListEvent) => setIsMd(event.matches);
    const handleLgChange = (event: MediaQueryListEvent) => setIsLg(event.matches);
    const handleXlChange = (event: MediaQueryListEvent) => setIsXl(event.matches);
    const handle2xlChange = (event: MediaQueryListEvent) => setIs2xl(event.matches);
